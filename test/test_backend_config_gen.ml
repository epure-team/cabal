...
let test_attribution backend_id () =
  ...
  Alcotest.(check bool)
    ...
    true
    (contains_str a.Backend_config_gen.content "Generated by Epure")
...
let test_claude_code_path_under_epure () =
  let a = get_artifact "claude-code" in
  Alcotest.(check bool)
    "claude-code path starts with .epure/"
    true
    (contains_str a.Backend_config_gen.project_relative_path ".epure/")
...
let test_setup_project_config_gemini_writes_settings_json () =
  with_tmpdir (fun dir ->
      ...
      Alcotest.(check bool)
        ...
        true
        (Sys.file_exists (expected ^ ".epure-meta.json")) ;
