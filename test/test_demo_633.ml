...
let test_copilot_cli_capability_evidence_none () =
  ...
  | Some _ ->
      Alcotest.fail
        "capability_evidence is Some _ for copilot-cli — unexpected on false flag"
...
let test_other_backends_still_false () =
  let others = ["codex"; "opencode"; "gemini-cli"] in
  List.iter
    (fun id ->
...
let () =
  Alcotest.run
    "Story #633 — Native JSON schema for copilot-cli (AC2(b) documented non-support)"
    [
