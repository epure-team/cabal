...
let test_gemini_cli_capability_evidence_none () =
  ...
  | Some _ ->
      Alcotest.fail
        "capability_evidence is Some _ for gemini-cli — unexpected on false flag"
...
let test_other_backends_still_false () =
  let others = ["codex"; "opencode"; "copilot-cli"] in
  List.iter
    (fun id ->
...
let () =
  Alcotest.run
    "Story #632 — Native JSON schema for gemini-cli (AC2(b) documented non-support)"
    [
