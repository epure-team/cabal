...
(** Tests for Story #631 — Native JSON schema wiring for opencode (AC2(b)).

    Outcome: documented non-support.  At baseline_version 1.14.20, the opencode
    CLI is provider-agnostic and does not expose a stable, machine-readable
    schema-forwarding surface that can be wired without a pinned provider.  Per
    Decision D-14, the "refuse-at-call-time" shape routes to AC2(b) because the
    descriptor cannot evidence schema enforcement at baseline_version without a
    pinned provider (Story #628 evidence contract).  The three-artifact contract
    (NFR-R1) is satisfied:
    ...
    - AC1: Investigation note exists at the canonical path, cites
           baseline_version 1.14.20, and records at least one source URL.
    - AC2(b): opencode descriptor has native_json_schema_output = false
              (NFR-R1 pinning test).
    - AC2(b): opencode has capability_evidence = None.
...
let investigation_note_path () =
  Filename.concat
    (project_root ())
    "libs/cabal/docs/native-json-schema-investigation/opencode.md"
...
(** AC1: The investigation note must exist at the canonical path for opencode. *)
...
(** AC1: The investigation note must cite the baseline_version (1.14.20) so
...
let test_investigation_note_cites_baseline_version () =
  ...
  if not (Sys.file_exists path) then
    ...
    Alcotest.(check bool)
      "investigation note cites baseline_version 1.14.20"
      true
      (contains_substring content "1.14.20")
  end
...
(** {1 AC2(b) — opencode descriptor pinning} *)

(** AC2(b) / NFR-R1: opencode must retain native_json_schema_output = false.
    This is the pinning test required by the three-artifact AC2(b) contract. *)
let test_opencode_native_json_schema_output_false () =
  let d = find_desc "opencode" in
  Alcotest.(check bool)
    "native_json_schema_output = false for opencode (pinning test)"
    false
    d.Backend_registry.capabilities.Backend_registry.native_json_schema_output

(** AC2(b): opencode must have capability_evidence = None.
...
(** QG-1: For every built-in backend with native_json_schema_output = true,
    ...
    Ensures Story #631 does not introduce a regression. *)
...
(** NFR-U2: Story #631 must not touch claude-code's descriptor.
  ...
  Alcotest.run
    ...
    [
      ( "AC1 investigation note",
        [
          Alcotest.test_case
            "investigation note exists for opencode"
            `Quick
      ...
      ( "AC2(b) opencode descriptor pinning",
        [
          Alcotest.test_case
            "native_json_schema_output = false for opencode"
            `Quick
            test_opencode_native_json_schema_output_false;
          Alcotest.test_case
            "capability_evidence = None for opencode"
            `Quick
            test_opencode_capability_evidence_none;
        ] );
