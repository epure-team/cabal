...
let digest content = Digestif.SHA256.(to_hex (digest_string content))
...
let with_workspace f =
  ...
  Fun.protect ~finally:(fun () -> remove_tree workspace) (fun () -> f workspace)

let attachment ?(id = "image") ?(path = "image.png")
    ?(media_type = Backend_types.Png) ?sha256 ?size_bytes content :
...
let expect_input_error expected result =
  ...
  | Error error ->
      Alcotest.failf "unexpected preflight error: %s"
...
let check_inputs_ok ~limits task =
  ...
  | Ok () -> ()
  | Error error ->
      Alcotest.failf "expected valid inputs, got: %s"
        (Task_preflight.render_error error)

let test_valid_png () =
  with_workspace @@ fun workspace ->
  write_file (Filename.concat workspace "image.png") png;
  check_inputs_ok ~limits:default_limits
...
let test_exact_multi_chunk_read_budget () =
  ...
  in
  check_inputs_ok ~limits (spec workspace ~attachments:[ attachment content ])
...
let test_zero_size_stops_without_probe_read () =
  ...
  let zero_limits : Task_preflight.limits =
    { max_attachments = 1; max_file_size_bytes = 0; max_total_size_bytes = 0 }
  in
  expect_input_error
    (function Task_preflight.Media_type_mismatch _ -> true | _ -> false)
    (Task_preflight.validate_inputs ~limits:zero_limits
...
let test_missing_attachment () =
  ...
  expect_input_error
    ...
    (Task_preflight.validate_inputs ~limits:default_limits
       (spec workspace ~attachments:[ attachment png ]))
      ...
      (fun () ->
        ...
        if not chmod_succeeded then Alcotest.skip ()
          ...
          if not unreadable then Alcotest.skip ()
            ...
            expect_input_error
              (function
                | Task_preflight.Attachment_unreadable _ -> true | _ -> false)
              (Task_preflight.validate_inputs ~limits:default_limits
                 (spec workspace ~attachments:[ attachment png ])))
...
let test_parent_path_outside_workspace () =
  ...
  expect_input_error
    (function
      | Task_preflight.Attachment_outside_workspace _ -> true | _ -> false)
    (Task_preflight.validate_inputs ~limits:default_limits
       (spec workspace ~attachments:[ attachment ~path png ]))
...
let find_streaming_descriptor ~parent_pid target_stat =
  ...
  |> Array.find_map (fun entry ->
      ...
      then None
        ...
        with Unix.Unix_error _ | Sys_error _ | Failure _ -> None)

let write_ready descriptor =
  let rec loop () =
    try ignore (Unix.write_substring descriptor "r" 0 1)
...
let run_during_attachment_stream ~target mutate validate =
  if not (Sys.file_exists "/proc/self/fd" && Sys.file_exists "/proc/self/fdinfo")
  then Alcotest.skip ()
  else
    ...
    | child_pid ->
        ...
        Fun.protect
          ...
          (fun () ->
            ...
            | Unix.WEXITED 0 -> ()
            | Unix.WEXITED code ->
                Alcotest.failf "stream mutation monitor exited with status %d"
                  code
            | Unix.WSIGNALED signal | Unix.WSTOPPED signal ->
                Alcotest.failf "stream mutation monitor received signal %d"
                  signal);
            result)
...
let test_attachment_ancestor_rename_during_stream_rejected () =
  ...
  run_during_attachment_stream ~target
    (fun () -> Unix.rename ancestor moved)
    (fun () ->
      Task_preflight.validate_inputs ~limits:streaming_limits
        (spec workspace
           ~attachments:
             [
               attachment ~path:"images/image.png" ~sha256:streaming_png_digest
                 streaming_png;
             ]))
...
let test_content_mutation_during_stream_rejected () =
  ...
  in
  run_during_attachment_stream ~target mutate (fun () ->
      Task_preflight.validate_inputs ~limits:streaming_limits
        (spec workspace ~attachments:[ streaming_attachment () ]))
  |> expect_stream_mutation_rejected
...
let proc_path_for_open_descriptor descriptor_stat =
  ...
  if not (Sys.file_exists proc_fds) then Alcotest.skip ()
    ...
    match
      Sys.readdir proc_fds |> Array.to_list
...
let test_incoherent_limits_rejected () =
  with_workspace @@ fun workspace ->
  let limits : Task_preflight.limits =
    { max_attachments = 1; max_file_size_bytes = 10; max_total_size_bytes = 5 }
  in
  expect_input_error
...
let test_attachment_count_limit () =
  ...
  expect_input_error
    ...
    (Task_preflight.validate_inputs ~limits
       (spec workspace ~attachments:[ attachment png ]))
...
let test_total_size_limit () =
  ...
  let attachments =
    [
      attachment ~id:"one" ~path:"one.png" png;
      attachment ~id:"two" ~path:"two.png" png;
    ]
  in
  let limits =
    {
...
let test_rendered_errors_do_not_leak_paths_or_bytes () =
  ...
  in
  Alcotest.(check bool)
    "absolute path not rendered" false
  ...
  in
  Alcotest.(check bool)
    "attachment bytes not rendered" false
  ...
  in
  Alcotest.(check bool)
    "raced attachment identifier not rendered" false
    (contains race_rendered secret_path)

let test_prepared_inputs_seal_bytes_modes_and_cleanup () =
  with_workspace @@ fun workspace ->
  with_outside_file (png ^ "outside replacement") @@ fun outside ->
  let original_png = Filename.concat workspace "image.png" in
  let original_jpeg = Filename.concat workspace "image.jpeg" in
  write_file original_png png;
  write_file original_jpeg jpeg;
  let task =
    spec workspace
      ~attachments:
        [
          attachment png;
          attachment ~id:"jpeg" ~path:"image.jpeg"
            ~media_type:Backend_types.Jpeg jpeg;
        ]
  in
  let prepared =
    match Task_preflight.prepare_inputs ~limits:default_limits task with
    | Ok prepared -> prepared
    | Error error -> Alcotest.fail (Task_preflight.render_error error)
  in
  let staged = Task_preflight.Private.staged_attachments prepared in
  let staging_directory =
    match Task_preflight.Private.staging_directory prepared with
    | Some path -> path
    | None -> Alcotest.fail "attachments produced no staging directory"
  in
  Fun.protect
    ~finally:(fun () ->
    ...
    (fun () ->
      Alcotest.(check int)
        "one staged file per attachment" 2 (List.length staged);
      Alcotest.(check int)
        "private task directory mode" 0o700
        (Unix.stat staging_directory).st_perm;
      List.iter
        (fun (_attachment, path) ->
          Alcotest.(check bool)
            "staged path is outside the mutable workspace" false
            (String.starts_with ~prefix:(workspace ^ Filename.dir_sep) path);
          Alcotest.(check int) "staged file mode" 0o600 (Unix.stat path).st_perm)
        staged;
      let png_path, jpeg_path =
        match staged with
        | [ (_, png_path); (_, jpeg_path) ] -> (png_path, jpeg_path)
        | _ -> Alcotest.fail "unexpected staged attachment set"
      in
      Alcotest.(check bool)
        "PNG transport file keeps a PNG extension" true
        (String.ends_with ~suffix:".png" png_path);
      Alcotest.(check bool)
        "JPEG transport file keeps a JPEG extension" true
        (String.ends_with ~suffix:".jpg" jpeg_path);
      Unix.rename original_png (original_png ^ ".old");
      Unix.symlink outside original_png;
      Sys.remove original_jpeg;
      Alcotest.(check string) "sealed PNG bytes" png (read_file png_path);
      Alcotest.(check string) "sealed JPEG bytes" jpeg (read_file jpeg_path));
  List.iter
    (fun (_attachment, path) ->
      Alcotest.(check bool) "staged file cleaned" false (Sys.file_exists path))
    staged;
  Alcotest.(check bool)
    "private task directory cleaned" false
    (Sys.file_exists staging_directory)

let test_staging_write_failure_is_sanitized_and_cleaned () =
  with_workspace @@ fun workspace ->
  let original = Filename.concat workspace "private-image.png" in
  write_file original png;
  let staging_directory = ref None in
  let result =
    Task_preflight.Private.prepare_inputs_with_hooks
      ~on_staging_directory:(fun path -> staging_directory := Some path)
      ~on_staged_file:(fun _path descriptor -> Unix.close descriptor)
      ~limits:default_limits
      (spec workspace ~attachments:[ attachment ~path:"private-image.png" png ])
  in
  let rendered =
    match result with
    | Error
        (Task_preflight.Input Task_preflight.Attachment_staging_failed as error)
      ->
        Task_preflight.render_error error
    | Error error -> Alcotest.fail (Task_preflight.render_error error)
    | Ok prepared ->
        ignore (Task_preflight.release_inputs prepared);
        Alcotest.fail "closed staging descriptor unexpectedly succeeded"
  in
  List.iter
    (fun secret ->
      Alcotest.(check bool)
        "staging failure omits sensitive material" false
        (contains rendered secret))
    [ original; png; digest png ];
  match !staging_directory with
  | Some path ->
      Alcotest.(check bool)
        "partial staging directory cleaned" false (Sys.file_exists path)
  | None -> Alcotest.fail "staging hook did not observe a directory"

let test_cleanup_failure_is_sanitized_and_retryable () =
  with_workspace @@ fun workspace ->
  write_file (Filename.concat workspace "image.png") png;
  let prepared =
    match
      Task_preflight.prepare_inputs ~limits:default_limits
        (spec workspace ~attachments:[ attachment png ])
    with
    | Ok prepared -> prepared
    | Error error -> Alcotest.fail (Task_preflight.render_error error)
  ...
  let staging_directory =
    ...
    | None -> Alcotest.fail "attachments produced no staging directory"
  in
  let blocker = Filename.concat staging_directory "cleanup-blocker" in
  Unix.mkdir blocker 0o700;
  let rendered =
    match Task_preflight.release_inputs prepared with
    | Error
        (Task_preflight.Input Task_preflight.Attachment_cleanup_failed as error)
      ->
        Task_preflight.render_error error
    | Error error -> Alcotest.fail (Task_preflight.render_error error)
    | Ok () -> Alcotest.fail "non-empty staging directory cleanup succeeded"
  in
  List.iter
    (fun secret ->
      Alcotest.(check bool)
  ...
  | Error error -> Alcotest.fail (Task_preflight.render_error error));
  Alcotest.(check bool)
    "cleanup retry removes private directory" false
    (Sys.file_exists staging_directory)
...
let malformed_feature_evidence_cases =
  [
    ...
    ("empty E2E notes/test reference", { feature_evidence with notes = " " });
    ( "empty Manual_probe command",
      { feature_evidence with test_method = Backend_types.Manual_probe " " } );
    ("empty evidence_url", { feature_evidence with evidence_url = Some " " });
  ]
      ...
      capabilities =
        {
          base.capabilities with
          media_support =
            { media_types = [ Backend_types.Png ]; evidence = None };
        };
    }
  in
  expect_capability_error
    (function
      | Task_preflight.Media_support_without_evidence -> true | _ -> false)
...
let test_positive_web_support_requires_evidence () =
  ...
  expect_capability_error
    (function
      | Task_preflight.Web_support_without_evidence -> true | _ -> false)
    (validate_capabilities descriptor (spec "/tmp"))
...
let test_every_requested_media_type_is_required () =
          ...
          media_support =
            {
              ...
              evidence = Some feature_evidence;
            };
        };
    }
  in
  ...
  in
  Alcotest.(check bool)
    "supported media type accepted" true
    (validate_capabilities descriptor
       (spec "/tmp" ~attachments:[ attachment png ])
    = Ok ());
  expect_capability_error
    (function
      | Task_preflight.Unsupported_media_type Backend_types.Jpeg -> true
      | _ -> false)
...
let test_web_access_hierarchy () =
  ...
  Alcotest.(check bool)
    "search accepted by search-and-fetch support" true
    (validate_capabilities fetch_descriptor
       (spec "/tmp" ~web_access:Backend_types.Web_search)
    = Ok ());
  let search_descriptor = with_maximum Backend_types.Web_search in
  Alcotest.(check bool)
    "search accepted by search support" true
    (validate_capabilities search_descriptor
       (spec "/tmp" ~web_access:Backend_types.Web_search)
    = Ok ());
  expect_capability_error
...
let test_read_only_gate () =
  ...
  in
  Alcotest.(check bool)
    "read-only request accepted when supported" true
...
let test_session_resume_gate () =
  ...
  in
  Alcotest.(check bool)
    "session resume accepted when supported" true
...
let input_tests =
  [
    ...
    ( "zero-size input stops without probe",
      `Quick,
      test_zero_size_stops_without_probe_read );
    ...
    ( "relative symlink inside accepted",
      `Quick,
      test_intra_workspace_symlink_accepted );
    ( "absolute symlink inside accepted",
      `Quick,
      test_absolute_intra_workspace_symlink_accepted );
    ( "deleted opened descriptor rejected",
      `Quick,
      test_deleted_open_descriptor_rejected );
    ( "attachment ancestor rename during stream",
      `Quick,
      test_attachment_ancestor_rename_during_stream_rejected );
    ( "workspace rename during stream",
      `Quick,
      test_workspace_rename_during_stream_rejected );
    ( "content mutation during stream",
      `Quick,
      test_content_mutation_during_stream_rejected );
    ( "workspace prefix collision rejected",
      `Quick,
      test_workspace_prefix_collision_rejected );
    ...
    ( "rendered errors are sanitized",
      `Quick,
      test_rendered_errors_do_not_leak_paths_or_bytes );
    ( "prepared inputs seal bytes, modes, and cleanup",
      `Quick,
      test_prepared_inputs_seal_bytes_modes_and_cleanup );
    ( "staging write failure is sanitized and cleaned",
      `Quick,
      test_staging_write_failure_is_sanitized_and_cleaned );
    ( "cleanup failure is sanitized and retryable",
      `Quick,
      test_cleanup_failure_is_sanitized_and_retryable );
  ]

let capability_tests =
  [
    ( "positive media support requires evidence",
      `Quick,
      test_positive_media_support_requires_evidence );
    ( "positive web support requires evidence",
      `Quick,
      test_positive_web_support_requires_evidence );
    ( "malformed media feature evidence",
      `Quick,
      test_malformed_media_feature_evidence );
    ( "malformed web feature evidence",
      `Quick,
      test_malformed_web_feature_evidence );
    ("valid feature evidence", `Quick, test_valid_feature_evidence);
    ( "native schema support still requires evidence",
      `Quick,
      test_native_schema_support_still_requires_evidence );
    ( "malformed native evidence version is rejected",
      `Quick,
      test_malformed_native_evidence_version_is_rejected );
    ( "every requested media type is required",
      `Quick,
      test_every_requested_media_type_is_required );
...
let () =
  Alcotest.run "Task_preflight"
    [
      ("input validation", input_tests);
      ("capability validation", capability_tests);
    ]
