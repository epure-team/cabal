...
let json_eq = Alcotest.testable (Fmt.of_to_string Yojson.Safe.to_string) Yojson.Safe.equal
...
let leaks needle json =
  ...
  let rec scan i =
    ...
    then false
    else if String.sub s i (String.length needle) = needle
    then true
    else scan (i + 1)
  in
  scan 0
...
let test_prompt_still_redacted () =
  let j = `Assoc [ "type", `String "user_message"; "prompt", `String "hello world" ] in
...
let test_jwt_field_redacted () =
  assert_redacted ~field:"jwt" ~secret:"eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJqb2UifQ.sig"
...
let test_url_with_credentials_redacted () =
  ...
  let j = `Assoc [ "url", `String "postgresql://admin:hunter2@db.local/prod" ] in
...
let test_nested_prompt_inside_array_redacted () =
  let j =
    `Assoc
      [ "messages"
      , `List
          [ `Assoc [ "role", `String "user"; "prompt", `String "deep secret" ] ]
      ]
  in
...
let () =
  Alcotest.run
    ...
    [ ( "baseline"
      , [ Alcotest.test_case "prompt still redacted" `Quick test_prompt_still_redacted
        ; Alcotest.test_case "event type preserved" `Quick test_event_type_preserved
        ] )
    ; ( "sensitive_fields"
      , [ Alcotest.test_case "environment" `Quick test_environment_field_redacted
        ...
        ; Alcotest.test_case "oauth_token" `Quick test_oauth_token_field_redacted
        ; Alcotest.test_case "jwt" `Quick test_jwt_field_redacted
        ; Alcotest.test_case "bearer" `Quick test_bearer_field_redacted
        ; Alcotest.test_case "connection_string" `Quick test_connection_string_redacted
