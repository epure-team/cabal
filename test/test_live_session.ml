...
let test_new_session_argv () =
  Alcotest.(check (list string))
    "full form"
    [ "tmux"; "new-session"; "-d"; "-s"; "s"; "-x"; "200"; "-y"; "50"; "-c"; "/tmp"; "claude" ]
...
let test_send_argv () =
  Alcotest.(check (list string)) "set-buffer (named, -- guards text)"
    [ "tmux"; "set-buffer"; "-b"; "s"; "--"; "multi\nline" ]
    (set_buffer_argv ~name:"s" "multi\nline");
  Alcotest.(check (list string)) "paste-buffer (delete after)"
    [ "tmux"; "paste-buffer"; "-d"; "-b"; "s"; "-t"; "s" ]
    (paste_buffer_argv ~name:"s");
  Alcotest.(check (list string)) "enter"
    [ "tmux"; "send-keys"; "-t"; "s"; "Enter" ]
    (enter_argv ~name:"s")

let test_query_argv () =
  Alcotest.(check (list string)) "capture" [ "tmux"; "capture-pane"; "-t"; "s"; "-p" ] (capture_argv ~name:"s");
  Alcotest.(check (list string)) "has-session" [ "tmux"; "has-session"; "-t"; "s" ] (has_session_argv ~name:"s");
  Alcotest.(check (list string)) "kill-session" [ "tmux"; "kill-session"; "-t"; "s" ] (kill_session_argv ~name:"s");
  Alcotest.(check (list string)) "list-sessions" [ "tmux"; "list-sessions"; "-F"; "#{session_name}" ] (list_sessions_argv ())
...
let () =
  Alcotest.run "live_session"
    [
      ...
      ("integration", [ Alcotest.test_case "lifecycle" `Quick test_live_lifecycle ]);
    ]
