...
let test_roundtrip () =
  ...
  Alcotest.(check bool) "roles and texts preserved" true
...
let test_conversation_only () =
  ...
  let evs =
    [
      make_event User "u";
      make_event ~tool:{ name = "Bash"; input_summary = "ls"; output_summary = "" } Tool "";
      make_event System "sys";
      make_event Assistant "a";
    ]
  in
  let back = Session_ingest.claude_code (Session_render.claude_code evs) in
  Alcotest.(check (list string)) "only user+assistant survive" [ "u"; "a" ]
    (List.map (fun e -> e.text) back)

let test_valid_jsonl () =
  let jsonl = Session_render.claude_code [ make_event User "x"; make_event Assistant "y" ] in
  List.iter
    (fun line ->
...
let test_empty () =
  Alcotest.(check string) "empty input yields empty string" ""
    (Session_render.claude_code [])

let () =
  Alcotest.run "session_render"
    [
      ( "claude_code",
        [
