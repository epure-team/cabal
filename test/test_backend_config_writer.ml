...
let test_write_fresh_file_succeeds () =
  with_project_dir (fun project ->
    ...
    let result = W.write_artifact ~project_dir:project ~force:false artifact in
    match result with
    | Written p ->
  ...
  with_project_dir (fun project ->
    ...
    | Backed_up_and_written { path; backup_path } ->
      ...
      Alcotest.(check bool)
        ...
        true
        (try
           let _ = Str.search_forward (Str.regexp_string "first=v2") body 0 in
           true
