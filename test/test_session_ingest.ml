...
let test_ingest_shapes () =
  ...
  Alcotest.(check int) "event count (summary+garbage skipped, thinking dropped)"
    4 (List.length evs);
  Alcotest.(check bool) "thinking text never surfaces" true
  ...
  | first :: _ ->
      ...
      Alcotest.(check (option string)) "provenance client" (Some "claude-code")
        first.provenance.client;
      Alcotest.(check (option string)) "provenance session" (Some "s1")
...
let test_ingest_assistant_and_tools () =
  ...
  Alcotest.(check (option string)) "assistant model" (Some "opus")
    assistant.model;
  (match assistant.tokens with
  | Some c -> Alcotest.(check (option int)) "input tokens" (Some 10) c.tokens_input
  ...
  let tool_use = List.find (fun e -> match e.tool with Some t -> t.name = "Bash" | None -> false) evs in
  (match tool_use.tool with
  | Some t -> Alcotest.(check bool) "input captured" true (String.length t.input_summary > 0)
  | None -> Alcotest.fail "expected tool ref")

let test_empty_and_blank () =
  Alcotest.(check int) "empty input" 0 (List.length (Session_ingest.claude_code ""));
  Alcotest.(check int) "blank lines" 0 (List.length (Session_ingest.claude_code "\n\n  \n"))
...
let test_never_raises_on_odd_json () =
  ...
  Alcotest.(check bool) "good block survived bad sibling" true (List.mem "good" ts);
  Alcotest.(check bool) "survivor record ingested" true (List.mem "survivor" ts)

let () =
  Alcotest.run "session_ingest"
    [
      ( "claude_code",
        [
          Alcotest.test_case "record shapes" `Quick test_ingest_shapes;
          Alcotest.test_case "assistant + tools" `Quick test_ingest_assistant_and_tools;
          Alcotest.test_case "empty/blank" `Quick test_empty_and_blank;
          Alcotest.test_case "never raises on odd json" `Quick test_never_raises_on_odd_json;
        ] );
    ]
