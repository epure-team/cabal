...
(** Tests for Story #632 — Native JSON schema wiring for gemini-cli (AC2(b)).
    ...
    Outcome: documented non-support.  At baseline_version 0.38.2, the
    gemini-cli does not expose a CLI surface for forwarding a JSON schema or
    [generationConfig] payload into the Gemini API invocation — no
    [--generation-config-file], [--response-schema], or [--response-mime-type]
    flag exists.  The three-artifact contract (NFR-R1) is satisfied:
    ...
    - AC1: Investigation note exists at the canonical path, cites
           baseline_version 0.38.2, and records at least one source URL and
           an accessed-on date (NFR-U3).
    - AC2(b): gemini-cli descriptor has native_json_schema_output = false
              (NFR-R1 pinning test).
    - AC2(b): gemini-cli has capability_evidence = None.
    ...
    - NFR-U2: Scope isolation — Story #632 must not touch other backends;
...
let investigation_note_path () =
  ...
  let standalone =
    Filename.concat root "docs/native-json-schema-investigation/gemini-cli.md"
  in
...
(** AC1: The investigation note must exist at the canonical path for gemini-cli. *)
...
(** AC1: The investigation note must cite the baseline_version (0.38.2) so
...
let test_investigation_note_cites_baseline_version () =
  ...
  if not (Sys.file_exists path) then
    ...
    Alcotest.(check bool)
      "investigation note cites baseline_version 0.38.2"
      true
      (contains_substring content "0.38.2")
  end
...
(** {1 AC2(b) — gemini-cli descriptor pinning} *)

(** AC2(b) / NFR-R1: gemini-cli must retain native_json_schema_output = false.
    This is the pinning test required by the three-artifact AC2(b) contract. *)
let test_gemini_cli_native_json_schema_output_false () =
  let d = find_desc "gemini-cli" in
  Alcotest.(check bool)
    "native_json_schema_output = false for gemini-cli (pinning test)"
    false
    d.Backend_registry.capabilities.Backend_registry.native_json_schema_output

(** AC2(b): gemini-cli must have capability_evidence = None.
    Carrying evidence on a false flag would be misleading. *)
let test_gemini_cli_capability_evidence_none () =
  match Backend_registry.get_capability_evidence "gemini-cli" with
  | None -> ()
  | Some _ ->
      Alcotest.fail
        "capability_evidence is Some _ for gemini-cli — unexpected on false \
         flag"
...
(** QG-1: For every built-in backend with native_json_schema_output = true,
    ...
    Ensures Story #632 does not introduce a regression. *)
...
(** NFR-U2: Story #632 must not touch claude-code's descriptor.
...
let test_claude_code_still_native_true () =
  ...
  Alcotest.(check bool)
    "claude-code retains native_json_schema_output = true (not touched by #632)"
    true
    d.Backend_registry.capabilities.Backend_registry.native_json_schema_output

(** NFR-U2: Other pending backends must retain native_json_schema_output = false. *)
let test_other_backends_still_false () =
  let others = ["opencode"; "copilot-cli"] in
  List.iter
    (fun id ->
      let d = find_desc id in
      Alcotest.(check bool)
        (id ^ " retains native_json_schema_output = false (not touched by #632)")
        false
...
let () =
  Alcotest.run
    "Story #632 — Native JSON schema for gemini-cli (AC2(b) documented \
     non-support)"
    [
      ( "AC1 investigation note",
        [
          Alcotest.test_case
            "investigation note exists for gemini-cli"
            `Quick
      ...
      ( "AC2(b) gemini-cli descriptor pinning",
        [
          Alcotest.test_case
            "native_json_schema_output = false for gemini-cli"
            `Quick
            test_gemini_cli_native_json_schema_output_false;
          Alcotest.test_case
            "capability_evidence = None for gemini-cli"
            `Quick
            test_gemini_cli_capability_evidence_none;
        ] );
