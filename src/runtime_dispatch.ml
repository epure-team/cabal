...
type runtime_snapshot = {entry : Runtime_entry.t}
...
let dispatch_failure_with_progress ~progress ~total_elapsed failure =
  ...
  | attempts ->
      let execution =
        make_terminal_execution ~attempts ~total_elapsed
...
let authorize_context prepared context =
  Task_execution_context.Private.authorize_transport context
    ...
    ~attachment_references:prepared.spec.Backend_types.attachments
    ~web_access_policy:prepared.spec.web_access ~prepared_inputs:prepared.inputs

let make_fallback_context ~sw =
...
let prepare_internal ~sw ~env ~limits ~backend_id ?expected_entry ?context spec =
...
let prepare ~sw ~env ~limits ~backend_id ?context spec =
  prepare_internal ~sw ~env ~limits ~backend_id ?context spec

let prepare_with_entry ~sw ~env ~limits ~backend_id ~expected_entry ?context
    spec =
  prepare_internal ~sw ~env ~limits ~backend_id ~expected_entry ?context spec
              ...
              match
                protect Backend_execution_failed (fun () ->
                    Json_schema_enforcer.Private.run_task_detailed ~sw ~env
                      ...
                      ~backend:prepared.runtime.entry.backend prepared.spec)
              with
              | Error failure ->
...
let execute_prepared_detailed ~sw ~env ?context ?on_raw_line prepared =
  ...
  execute_prepared_detailed_with_progress ~sw ~env ?context ?on_raw_line
    ~progress prepared

let execute_prepared ~sw ~env ?context ?on_raw_line prepared =
  execute_prepared_detailed ~sw ~env ?context ?on_raw_line prepared
  |> project_detailed_outcome_with_context ~has_context:(Option.is_some context)
  ...
  Eio.Fiber.fork ~sw (fun () ->
      ...
      Eio.Cancel.sub (fun cancellation ->
          ...
          try
            let result, context =
              match
                Task_deadline.create mono_clock spec.Backend_types.timeout
              with
          ...
          with (Out_of_memory | Stack_overflow | Sys.Break) as fatal ->
            Eio.Cancel.protect (fun () ->
                (try
                   Task_event.emit_terminal sink
...
let start_task_with_entry ~sw ~env ~limits ~backend_id ~expected_entry ?on_event
    ?on_raw_line spec =
  start_task_with_preparer ~prepare_inputs:Task_preflight.prepare_inputs ~sw ~env
...
let await handle = await_detailed handle |> project_contextual_detailed_outcome
...
module Private = struct
  ...
  let prepare_with_input_hooks ~sw ~env ~limits ~backend_id ?context
      ?on_prepare_inputs ?on_staging_directory ?on_staged_file ?on_cleanup_attempt
      spec =
    let cleanup_status = Atomic.make Backend_types.Cleanup_not_required in
    prepare_with
      ~prepare_inputs:
  ...
  let start_task_with_entry = start_task_with_entry
  let cancel = cancel
  let await = await
...
let run_task_with_entry ~sw ~env ~limits ~backend_id ~expected_entry ?on_event
    ?on_raw_line spec =
  start_task_with_entry ~sw ~env ~limits ~backend_id ~expected_entry ?on_event
    ?on_raw_line spec
  |> await
...
let run_task_detailed_with_entry ~sw ~env ~limits ~backend_id ~expected_entry
