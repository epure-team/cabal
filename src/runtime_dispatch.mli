...
(** Central validated task invocation.
    ...
    and only then invokes {!Json_schema_enforcer.run_task}. Registry overrides
    installed after a dispatcher/completer is constructed are therefore visible
    on the next invocation. The resolved first-class backend is retained as one
...
(** [render_error error] produces a sanitized diagnostic. Preflight rendering
    ...
    low-level schema-enforcement strings are redacted before display. *)
val render_error : error -> string

(** [run_task ~sw ~env ~limits ~backend_id spec] performs central resolution,
    consistency validation, input/capability preflight, availability checking,
    and schema-enforced execution.

    [limits] is mandatory caller policy; Cabal supplies no product default.
    Validation and availability failures happen before [backend.run_task],
    project config generation, or the task process spawn.
