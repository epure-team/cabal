...
let attribution_from_comment_header header =
  let rec first_attribution = function
    | [] -> None
    | line :: rest ->
        let trimmed = String.trim line in
        if
          String.starts_with ~prefix:"//" trimmed
          && (not (contains_substr trimmed "epure-managed"))
          && not (contains_substr trimmed "epure-hash")
        then
          let raw = String.sub trimmed 2 (String.length trimmed - 2) in
          Some (String.trim raw)
        else first_attribution rest
  in
  first_attribution (String.split_on_char '\n' header)

let is_managed_comment_header header =
  contains_substr header "epure-managed" || contains_substr header "epure-hash"
...
let managed_header_for_body ~attribution body =
  let hash =
    Backend_config_writer.managed_body_hash ~backend_id:"opencode" body
  in
  Printf.sprintf "// %s\n// epure-managed\n// epure-hash: %s\n" attribution hash

let header_for_updated_body ~existing_content ~existing_fields body =
  let default_attribution = "Generated by Epure — do not edit manually" in
  match existing_content with
  | None -> ""
  | Some content ->
      let comment_header = leading_comment_header content in
      if is_managed_comment_header comment_header then
        let attribution =
          match attribution_from_comment_header comment_header with
          | Some s when s <> "" -> s
          | _ -> default_attribution
        in
        managed_header_for_body ~attribution body
      else if has_legacy_epure_fields existing_fields then
        let attribution =
          match List.assoc_opt "_epure_attribution" existing_fields with
          | Some (`String s) -> s
          | _ -> default_attribution
        in
        managed_header_for_body ~attribution body
