...
(** Project-owned backend configuration generation — Story #478.

    Generates backend-native project configuration files for each supported
    backend.  The generated artifacts are attributable to Épure, idempotent,
    and protected against silent overwrites of user-authored content.

    Two ownership models are supported:
    - {b Épure-owned}: the file lives under [.epure/backend-config/] and is
      passed explicitly to the backend at invocation via a CLI flag or env var.
      Épure freely regenerates these files.
    - {b Backend-project}: the backend requires a fixed project-relative path
      ...
      Épure writes there only when the file is absent or carries Épure managed
      metadata, and refuses to overwrite user-authored content silently. *)
...
type ownership = Backend_config_writer.ownership =
  | Epure_owned
      (** File lives under [.epure/backend-config/]; passed explicitly to the
          backend at invocation.  Épure freely regenerates these files. *)
  | Backend_project
      (** File lives at the backend's required fixed project path.  Épure
          writes only to managed files (those bearing the Épure marker + hash).
          Unmarked user files are skipped. *)
...
type artifact = Backend_config_writer.artifact = {
  ...
  ownership : ownership;
      (** Whether this file is Épure-owned or at a fixed backend path. *)
  ...
  content : string;
      (** Full file content.  Strict JSON artifacts may use sidecar metadata
          rather than inline Épure headers. *)
}
...
type write_result = Backend_config_writer.write_result =
  ...
  | Skipped_user_content of string
      (** A user-authored file (no Épure marker) exists at the path;
...
(** [is_managed_content content] returns [true] when [content] contains the
    Épure managed marker, indicating this file was generated by Épure.

    {pre}
    (none)

    {post}
    Returns [true] if and only if the Épure [epure-managed] marker appears
    as a substring of [content].
...
(** [extract_hash content] extracts the hex MD5 hash embedded by Épure in
    [content]'s managed header.  Returns [None] when no hash is present
    (e.g. Épure-owned files, or non-managed content).
    ...
    or [None] when no [epure-hash] marker is found in [content].
...
(** [managed_body_hash ~backend_id content] computes the hash Épure stores in
...
(** [generate ~backend_id] generates a project configuration artifact for the
    ...
    [.github/copilot-instructions.md]).  Returns [None] for unknown backends
...
(** [generate_all ~backend_id ~mcp_servers] generates every project
    ...
    [.github/mcp.json].  Strict JSON artifacts carry no inline Épure metadata;
    write-time ownership is tracked by a sidecar file.
...
(** [write_artifact ~project_dir ~force artifact] writes [artifact] to disk
    ...
    - File present, no Épure marker → returns [Skipped_user_content path].
    - File present, Épure marker, hash matches current body → content
      unchanged → [Already_current]; or overwrite with new template and
      return [Written path] if the template changed.
    - File present, Épure marker, hash mismatch → user modified the file.
      With [~force:false] returns [Refused_hash_mismatch msg].
      With [~force:true] backs up to [path ^ ".epure-backup"] and writes,
      returning [Backed_up_and_written].
