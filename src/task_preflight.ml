...
let render_input_error = function
  ...
  | Too_many_attachments {maximum; actual} ->
      Printf.sprintf
        "attachment count exceeds caller limit (maximum %d, actual %d)"
        maximum
        actual
  ...
  | Attachment_too_large {maximum; actual; _} ->
      Printf.sprintf
        "attachment exceeds per-file size limit (maximum %d, actual %d)"
        maximum
        actual
  | Total_size_too_large {maximum; actual} ->
      Printf.sprintf
        "attachments exceed total size limit (maximum %d, actual %d)"
        maximum
        actual
  | Malformed_sha256 _ ->
...
let validate_count limits attachments =
  ...
  if actual > limits.max_attachments then
    Error
      (Input
         (Too_many_attachments {maximum = limits.max_attachments; actual}))
  else Ok ()
...
let remaining_allowance ~limit ~used =
  if used >= limit then 0 else limit - used
...
let read_digest_and_magic ~limits ~total attachment descriptor =
  let buffer = Bytes.create 65536 in
  let magic = Bytes.create 8 in
  let remaining_total = limits.max_total_size_bytes - total in
  let rec read length =
    try Unix.read descriptor buffer 0 length with
  ...
  let rec loop context magic_length observed_size =
    if observed_size = attachment.Backend_types.size_bytes then
      ...
      let budget =
        next_read_budget
          ~buffer_capacity:(Bytes.length buffer)
          ~declared_size:attachment.size_bytes
          ~per_file_limit:limits.max_file_size_bytes
          ~remaining_total
          ~observed_size
      in
      if budget = 0 then
...
let attachment_changed attachment =
  Error
    (Input
       (Attachment_changed_during_validation attachment.Backend_types.id))

let open_attachment workspace attachment =
  try Ok (openat_readonly workspace.descriptor attachment.Backend_types.path)
  with
  ...
  try
    ...
    then attachment_changed attachment
      ...
      if descriptor_path_is_deleted attachment_path then
        ...
        path_is_contained
          ~workspace:workspace.resolved_path
          attachment_path
      then Ok attachment_path
      else Error (Input (Attachment_outside_workspace attachment.id))
  with
  | Unix.Unix_error _ | Invalid_argument _ -> attachment_changed attachment
...
let validate_open_attachment ~limits ~total ~workspace attachment =
        ...
        if initial_stat.st_nlink = 0 then attachment_changed attachment
          ...
          if attachment.size_bytes <> actual then
            ...
            then attachment_changed attachment
              ...
              else Ok (total + streamed.observed_size)
      with Unix.Unix_error _ ->
        Error (Input (Attachment_unreadable attachment.id)))

let validate_attachments ~limits ~workspace attachments =
  let rec loop total = function
    | [] -> Ok ()
    | attachment :: rest ->
        let* total =
          validate_open_attachment ~limits ~total ~workspace attachment
        in
        loop total rest
  in
  loop 0 attachments
...
let validate_inputs ~limits spec =
  ...
  with_workspace_descriptor spec.working_dir (fun workspace ->
      validate_attachments ~limits ~workspace attachments)
  ...
  match
    List.find_opt
      (fun attachment ->
        not
          (List.mem
             attachment.Backend_types.media_type
