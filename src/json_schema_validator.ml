...
let format_instance_location (loc : Jsonschema.instance_location) : string =
  ...
  | tokens ->
      let parts =
        List.map
          (function
            | Jsonschema.Prop s -> s | Jsonschema.Item i -> string_of_int i)
          tokens
      in
      "/" ^ String.concat "/" parts
...
let format_error_kind (kind : Jsonschema.error_kind) (loc : string) : string =
  ...
  | Jsonschema.Additional_properties {got} ->
      Printf.sprintf
        "additional properties not allowed at %s: [%s]"
        loc
        (String.concat ", " got)
  | Jsonschema.Unevaluated_properties {got} ->
      Printf.sprintf
        "unevaluated properties at %s: [%s]"
        loc
        (String.concat ", " got)
  ...
  | Jsonschema.Required {want} ->
      Printf.sprintf
        "missing required properties at %s: [%s]"
        loc
        (String.concat ", " want)
  | Jsonschema.Dependency {prop; missing} ->
      Printf.sprintf
        "dependency error at %s: property %S requires [%s]"
        loc
        prop
        (String.concat ", " missing)
  | Jsonschema.Dependent_required {prop; missing} ->
      Printf.sprintf
        "dependent required error at %s: property %S requires [%s]"
        loc
        prop
        (String.concat ", " missing)
  ...
  | Jsonschema.Min_contains {want; _} ->
      Printf.sprintf
        "minContains not satisfied at %s: need at least %d"
        loc
        want
  | Jsonschema.Max_contains {want; _} ->
      Printf.sprintf "maxContains not satisfied at %s: need at most %d" loc want
  ...
  | Jsonschema.Additional_items {got} ->
      Printf.sprintf
        "additional items not allowed at %s: %d extra item(s)"
        loc
        got
  | Jsonschema.Min_length {got; want} ->
      Printf.sprintf "string too short at %s: got %d chars, min %d" loc got want
