...
let validate_descriptor limits descriptor =
  ...
  | Error _ as err -> err
  | Ok () -> (
    match validate_token ~field:"resource title" descriptor.descriptor_title with
    | Error _ as err -> err
    | Ok () -> (
      match validate_token ~field:"resource kind" descriptor.descriptor_kind with
      | Error _ as err -> err
      | Ok () ->
        validate_all (validate_metadata limits) descriptor.descriptor_metadata ) )
...
let validate_resource limits resource =
  ...
  | Ok () -> (
    ...
    | Ok () -> (
      ...
      | Error _ as err -> err
      | Ok () -> (
        match validate_all (validate_metadata limits) resource.metadata with
        | Error _ as err -> err
...
let make ?(authorization_boundary = default_authorization_boundary)
  ...
  | Ok () -> (
    ...
    | Ok () -> (
      match
        validate_token ~field:"authorization boundary" authorization_boundary
      with
      | Error _ as err -> err
      | Ok () -> (
    ...
    | Ok () ->
      let rec loop acc index =
        if index > limits.max_windows_per_resource then
          ...
          | Error msg -> Error ("workspace_reader_error:" ^ bounded_reader_error msg)
          | Ok result -> (
            match validate_token ~field:"window citation_id" result.citation_id with
            | Error _ as err -> err
            | Ok () ->
              if String.length result.content > limits.read_max_window_chars then
                Error "workspace_reader_window_too_large"
              else
              let window =
                make_window ~citation_id:result.citation_id ~index ~total:index
              ...
              if result.has_more then loop (window :: acc) (index + 1)
                ...
                Ok (List.map (fun window -> {window with total}) windows) )
      in
      match loop [] 1 with
      | Error _ as err -> err
      | Ok windows ->
        Ok
          (make_resource ~id:descriptor.descriptor_id
             ~title:descriptor.descriptor_title ~kind:descriptor.descriptor_kind
             ~metadata:descriptor.descriptor_metadata ~windows ()) )
...
let collect_workspace ?authorization_boundary ?(limits = default_limits)
  ...
  let rec loop acc = function
    ...
    | descriptor :: rest -> (
      match collect_resource_windows ~limits:read_limits ~read_window descriptor with
      | Error _ as err -> err
  ...
  let resources =
    ...
    | resources ->
      ...
      |> List.map (fun resource ->
        ...
        in
        Printf.sprintf "- [%s] %s (%s), windows=%d%s" resource.id
          resource.title resource.kind (List.length resource.windows) metadata)
      |> String.concat "\n"
  in
  Printf.sprintf
    "# Virtual workspace: %s\n\
     ...
     Resource manifest:\n%s"
    workspace.label workspace.authorization_boundary resources

let render_window limits resource window =
  Printf.sprintf
    "### Resource [%s] %s — window %d/%d — citation %s\n%s" resource.id
    resource.title window.index window.total window.citation_id
...
let render_resource limits resource =
  ...
  | [] ->
    Printf.sprintf "## Resource [%s] %s\n(no windows supplied)" resource.id
      resource.title
  | windows ->
    windows
